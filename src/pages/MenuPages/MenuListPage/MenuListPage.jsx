        ...
        <div css={s.layout}>
            {menuModal === true ? (
                <MenuRegisterModal
                    ...
                    categoryName={categories.find(cat => cat.menuCategoryId === menuCategoryId)?.menuCategoryName || ""}
                    recommend={recommend}
            ...
            <div>
                <div css={s.filterLayout}>
                    <div style={{display:"flex"}}>
                        <h1>매장단품 메뉴관리</h1>
                        <button onClick={getMenuList} type="button">검색</button>  
                        <button onClick={initializeState} type="reset">초기화</button>
                        <button type="button" onClick={openMenuModal}>등록</button>
                        <button onClick={deleteMenu} type="button">삭제</button>

                    </div>
                    <div>
                        <div style={{display:"flex"}}>
                            <div name="storeName" >매장명</div>
                            <input type="text" id="storeName" name="storeName" value={"테스트 매장"} disabled />
                        </div>
                        <div style={{display:"flex"}}>
                            <label htmlFor="saleState">판매 상태</label>
                            <select value={menuState} onChange={(e) => setMenuState(e.target.value)} id="saleState" name="saleState">
                        ...
                        <div style={{display:"flex"}}>
                            ...
                            <select value={recommendMenu} onChange={(e) => setRecommendMenu(e.target.value)} id="saleState" name="saleState">
                                <option value="0">전체</option>
                        ...
                        <div>
                            ...
                            <select onChange={(e) => setInputState(e.target.value)} id="menuType" name="menuType">
                                <option value="1">메뉴코드</option>
                                <option value="2">메뉴이름</option>
                            </select>
                            <input value={inputState === "1" ? menuCode : searchMenuName} type="text" onChange={inputState === "1" ? handleSearchMenuCode : handleSearchMenuName} />
                        </div>
                ...
                <div css={s.menuListLayout}>
                    <table css={s.tableLayout}>
                        ...
                        <tbody>
                            {menuList?.map(menu => (
                                <tr key={menu.menuId}>
                                    <td><input type="checkbox" name="menuCheck" id="menuCheck" onChange={() => handleCheckboxChange(menu.menuId)}/></td>
                                    <td>{menu.menuId}</td>
                                    <td>{menu.companyName}</td>
                                    <td onClick={() => navigate(`/menu/management/detail/${menu.menuId}`)}>{menu.menuCode}</td>
                                    <td onClick={() => navigate(`/menu/management/detail/${menu.menuId}`)}>{menu.menuName}</td>
