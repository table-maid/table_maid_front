        ...
        {updateState === 0 && (
          <div css={s.optionButtonLayout}>
            ...
            <button
              css={s.optionButton}
              onClick={() =>
                handelDeleteOptionTitle(
                  optionItem.optionTitleId,
                  optionItem.optionTitleName
                )
              }
            >
              <MdDeleteForever />
              ...
              {updateState === 0 && (
                <div>
                  ...
                  <button
                    css={s.optionButton}
                    onClick={() =>
                      handelDeleteOption(optionItem.optionNameIds[idx])
                    }
                  >
                    <MdDeleteForever />
                  </button>
        ...
        <div>
          <div css={s.menuDetailLayout}>
            <div css={s.menuDetailContent}>
              ...
              <div css={s.selectWrapper}>
                ...
                <select
                  name="categoryId"
                  value={menuDetail?.menuCategoryId}
              ...
              <div css={s.checkboxContainer}>
                ...
                <input
                  type="checkbox"
                  name="recommendMenu"
                  checked={menuDetail?.recommendMenu === 2}
                  onChange={handleInputChange}
                />
              </div>
            </div>
            <div>
              <MenuImg
                adminId={adminId}
                menuImgUrl={menuDetail.menuImgUrl}
                menuId={menuId}
              />
            </div>
          </div>
          ...
          <div css={s.optionLayout}>
            <div>
              ...
              <div>
                * 옵션 제목 삭제 시 해당하는 옵션 내용이 모두 삭제되니
                주의해주세요.
              </div>
            </div>
            <div css={s.optionManagementLayout}>
              <div css={s.optionManagementTitleLayout}>
                {menuDetail.optionList?.map(
                  (optionItem, index) =>
                    optionItem &&
                    optionItem.optionTitleName && (
                      <OptionList key={index} optionItem={optionItem} />
                    )
                )}
              </div>
              {updateState !== 0 && (
                  ...
                  {updateState === 1 ? (
                    <div css={s.updateMenuLayout}>
                      ...
                      <input
                        ...
                        value={updateOptionData?.optionPrice}
                      />
                    </div>
                  ) : (
                    <div css={s.updateMenuLayout}>
